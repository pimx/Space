...
model Chamber "Rocket engine chamber with thrust vectoring"
    ...
    constant Real M_IMPULSE     = 325                     "импульс тяги [s]";
	//constant Real maxThrust     =  2.00 * Constants.M_dry * Constants.g0  "максимальная тяга 169 / SCALE^3 * M_G0 * M_IMPULSE [N]";
	...
	constant Real minThrust     =  1.15 * Constants.M_dry * Constants.g0  "минимальная  тяга  91 / SCALE^3 * M_G0 * M_IMPULSE [N]";
    constant Real gimbalLimit   = 0.2                     "максимальное отклонение вектора тяги [rad]";
	// Parameters
    //parameter Real specificImpulse = 325     "Specific impulse [s]";
    //parameter Real g0              = 9.80665 "Standard gravity [m/s^2]";
	parameter Real angle           = 0.0     "Orientation angle [rad]";
	parameter Real offset          = 0.0     "Radial distance of chambers from rocket centerline [m]";
...
//equation
	...
	chamberFuelRate = chamberMagnitude / (M_IMPULSE * Constants.g0);
